# Phase 4 - Version 2.0 : RAG Q&A System - Retrieve and Answer (with flan-t5 models, switchable between small, base and large)
# Equipped with enhanced engineering changes and context truncation handling.
import os
import faiss
import json
import torch
...
# Folder paths
DB_FOLDER = "vector_db1"
CHUNKS_FOLDER = "chunks1"
...
def generate_answer(question, context_chunks):
    ...
    context_tokens = qa_tokenizer(context, return_tensors="pt", truncation=False)["input_ids"]
    if context_tokens.shape[1] > 800:  # reserve ~200 tokens for question and answer
        context = qa_tokenizer.decode(context_tokens[0, :800], skip_special_tokens=True)

    # Strong prompt to force model to stick to the context
    prompt = (
